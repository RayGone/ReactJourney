...
const TaskValue: FC<PropsWithChildren> = ({children}) => 
         <div className=" w-full text-base border-1 border-l-4 shadow-[inset_1px_1px_3px_2px_rgba(0,0,0,0.1)] dark:shadow-[inset_1px_1px_3px_2px_rgba(0,0,0,0.2)] border-blue-500/30 p-2 rounded">{children}</div>
...
const Description: FC<Prop> = ({taskID}) => {
    ...
    return (<div className="block rounded w-full p-4 text-xl
        ...
        <h2 className="w-full text-2xl font-bold py-2 mb-3 border-b-1 text-shadow-sm text-shadow-blue-500/30">{task.name}</h2>
